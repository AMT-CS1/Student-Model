...
def ReadProblemQmatrix():
    ...
    with open(dirPath+'/PREDICTKCMC/data/ModelInput'+'/problemQmatrix.CSV', 'r', encoding='utf-8-sig', newline='') as fileInput:
...
def ReadPersonalizedQmatrix():
    ...
    with open(dirPath+'/PREDICTKCMC/data/ModelInput'+'/P-matrix-out.CSV', 'r', encoding='utf-8-sig', newline='') as fileInput:
        reader = csv.reader(fileInput)
        for row in reader:
...
def WriteNewLabel():
    for path in ['train.CSV', 'test.CSV']:
        dataDir = {}
        with open(dirPath+'/PREDICTKCMC/data/ModelInput/'+path, 'r', encoding='utf-8-sig', newline='') as fileInput:
            reader = csv.reader(fileInput)
            _ = 0
            for row in reader:
                dataDir[_] = row
            ...
            while i < len(dataDir.keys()):
                ...
                if count != 1:
                    dataDir[i] = tmp
                i += count
        with open(dirPath+'/PREDICTKCMC/data/ModelInput/'+path, 'w', encoding='utf-8-sig', newline='') as fileInput:
